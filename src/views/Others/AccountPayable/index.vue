              ...
              <tr v-for="(account, index) in paginatedData" :key="index"
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  <div class="text-slate-600 px-2 text-center rounded-xl bg-gray-400" :class="{
                    'bg-green-400': account.status_payment === 'full',
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div class="flex space-x-3">
                    ...
                    <button v-if="account.status_payment != 'unpaid'" @click="resetPrice(account)" class="text-yellow-600 hover:text-yellow-900">
                      Reset
                    </button>
                  </div>
                </td>
  ...
  setup() {
    ...
    const getArcheive = async () => {
      try {
        ...
        Jasakirim.value = data.jasakirim;        
      } catch (error) {
    ...
    const filteredData = computed(() => {
      let result = [...Purchase.value, ...Jasakirim.value]

      if(status.value === 'purchase'){
        result = Purchase.value;
      }else if(status.value === 'jasakirim'){
        result = Jasakirim.value;
      }
      ...
      if (item) {
        Swal.fire({
          ...
          if (res.isConfirmed) {
            ...
            ApiServices.put(AccPayableReset + '/' + item.id_po, {
              deposit: deposit,
