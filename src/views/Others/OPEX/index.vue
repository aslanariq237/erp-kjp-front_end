            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-gray-900">{{ opex.name }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{ formatCurrency(opex.amount) }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{ formatAccountNumber(opex.accountNumber) }}
                </td>
...
export default defineComponent({
  ...
  setup() {
    ...
    const filteredData = computed(() => {
      ...
      if (searchQuery.value) {
        ...
        result = result.filter(
          (opex) => opex.name.toLowerCase().includes(query) || opex.accountNumber.includes(query),
        )
      }

      if (minAmount.value) {
        result = result.filter((opex) => opex.amount >= minAmount.value)
      }

      if (maxAmount.value) {
        result = result.filter((opex) => opex.amount <= maxAmount.value)
      }
    ...
    const viewDetails = (opex) => {
      router.push(`/opex/${opex.id}`)
    }

    const editOpex = (opex) => {
      router.push(`/opex/edit/${opex.id}`)
    }
    ...
    const deleteOpex = async () => {
      try {
        ...
        // await api.delete(`/opex/${opexToDelete.value.id}`)

        // Remove from local state
        opexData.value = opexData.value.filter((opex) => opex.id !== opexToDelete.value.id)
    ...
    const exportData = () => {
      const data = filteredData.value.map((opex) => ({
        Name: opex.name,
        'Account Number': opex.accountNumber,
        Amount: formatCurrency(opex.amount),
        'Date Created': opex.dateCreated,
      }))
