...
export default defineComponent({
  ...
  setup() {
    ...
    const deleteAccount = async () => {
      try {
        ...
        accounts.value = accounts.value.filter(
          (account) => account.id_invoice !== accountToDelete.value.id_invoice,
        )
    ...
    const exportData = () => {
      ...
      a.setAttribute('download', `faktur-pajak-${new Date().toISOString().split('T')[0]}.csv`)
