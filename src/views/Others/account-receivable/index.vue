    ...
    methods: {
        ...
        async saveDeposit() {
            if (this.selectedItem) {
                try {
                    const deposit = Number(this.selectedItem.deposit || 0) + Number(this.additionalDeposit || 0);
                    ...
                    const response = await ApiServices.put(AccUpdateDeposit + '/' + this.selectedItem.id_invoice, {
                        id_invoice: this.selectedItem.id_invoice,
                        payment_method: 'Transfer',
                        status_payment: status_payment,
                        deposit: deposit,
                        issue_at: this.issue_today,
                        due_at: this.issue_today
                    });
        ...
        exportData() {
            const data = this.accounts.map((account) => {
                ...
                return {
                    ...
                    Aging: this.calculateDay(account.issue_at, account.due_at) || 0,
