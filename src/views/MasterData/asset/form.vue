            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="grid grid-cols-2 md:grid-cols-2 gap-3">
                    ...
                    <FormGroup 
                        label="Price" 
                        :required="true" 
                        :error="rules.price" errorMessage="Asset Price is required">
                        <input type="number" id="deposit" name="deposit" v-model="price"
                            :class="inputClass(rules.deposit)" placeholder="Enter Price" />
                    </FormGroup>
                    <FormGroup label="Asset Life" :required="false" :error="rules.deposit"
...
export default defineComponent({
    ...
    methods: {
        ...
        calculateDay() {
            ...
            const due_date = new Date(this.due_at);

            const timeDiff = due_date.getTime() - today.getTime();
            const dayDiff = Math.ceil(timeDiff / (1000 * 3600 * 24));
            this.umur_assets = dayDiff;
        },
        ...
        async validation() {
            ...
            if (this.assets_name == '' || this.assets_name == null) {
                this.rules.assets_name = true
                count++
            } else {
                this.rules.assets_name = false
            }

            if (this.price == '' || this.price == null) {
                this.rules.price = true
                count++
            } else {
                this.rules.price = false
            }

            return count
        },
        async getById(id) {
            await ApiServices.get(Asset + '/' + id).then(
                (res) => {
                    var data = res.data;
                    ...
                    this.price = data[0].assets_price;
