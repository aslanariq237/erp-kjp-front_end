        ...
        <div class="quotation p-20" v-for="(page, pageIndex) in pages" :key="pageIndex" :class="page">
            ...
            <div class="body">
                <div class="mt-3">
                    ...
                    <div class="mt-2">
                        <table class="min-w-full mt-4 divide-gray-100 shadow-xs border-gray-200 border">
                            <thead class="border">
                                <tr class="text-center">
                                    ...
                                    <th class="text-xs px-3 font-semibold">Amount</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-gray-800">
                                <tr class="text-center border-b-2" v-for="(pro, index) in page" :key="index">
                                    ...
                                    <td class="text-xs px-3 py-2 items-center">{{ pro.product.product_uom }}</td>
                                    <td class="text-xs px-3 py-2 items-center">{{ numberWithCommas(pro.price) }}</td>
                                    <td class="text-xs px-3 py-2 items-center">{{ numberWithCommas(pro.amount) }}</td>
                                </tr>
                            </tbody>
                        </table>
...
export default defineComponent({
    ...
    computed: {
        pages() {
            ...
            const result = [];
            for (let i = 0; i < this.item.detail_quo.length; i += rowsPerPage) {
                result.push(this.item.detail_quo.slice(i, i + rowsPerPage));
            }
            return result;
        }
    },
