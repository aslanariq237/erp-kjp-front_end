...
export default defineComponent({
    ...
    computed: {
        totalGrandTotal() {
            if (!this.item.detailtandater || !this.item.detailtandater.length) return 0;
            return this.item.detailtandater.reduce((sum, pro) => {
                // Check if invoice exists and grand_total is a valid number
                const grandTotal = pro.invoice ? parseFloat(pro.invoice.grand_total) || 0 : 0;
